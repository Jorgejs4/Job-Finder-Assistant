...
if db.get_job_count() == 0:
    ...
    if os.path.exists(json_path):
        ...
        print(f"[Dashboard] Migrados {result['jobs']} jobs desde data.json")
...
with tab_mis_ofertas:
    ...
    mis_ofertas = [j for j in all_jobs if not j.get("archived") and not j.get("needs_analysis")]
    ...
    if not mis_ofertas:
        ...
        with st.expander("🔍 Filtros avanzados", expanded=True):
            ...
            with f4:
                min_score = st.slider("Match minimo", 0, 100, 0)
            ...
            with f7:
                ...
                exp_filter = st.slider("Experiencia max (anios)", 0, max(max_exp, 10), max(max_exp, 10))
            with f8:
            ...
            with st.expander(" | ".join(header_parts)):
                ...
                if interview_prep:
                    ...
                    with st.expander("🎯 Preparacion para Entrevista", expanded=False):
                        ...
                        if not any([tech_qs, beh_qs, key_topics, tips]):
                            st.info("No hay contenido de preparacion disponible.")
                ...
                if cv_url:
                    ...
                    with st.form(key=f"fb_{jid}", clear_on_submit=True):
                        ...
                        if submitted and fb.strip():
                            ...
                            st.success("Feedback guardado. Se procesara en la proxima ejecucion.")
                            st.rerun()
                        elif submitted:
...
with tab_stats:
    ...
    if not all_jobs:
        ...
        if all_sal_data:
            ...
            sc1.metric("Promedio", f"{sum(all_sal_data) // len(all_sal_data):,}E".replace(",", "."))
            sc2.metric("Mediano", f"{statistics.median(all_sal_data):,.0f}E".replace(",", "."))
            sc3.metric("Minimo", f"{min(all_sal_data):,}E".replace(",", "."))
            sc4.metric("Maximo", f"{max(all_sal_data):,}E".replace(",", "."))
            ...
            with st.expander("Salario por modalidad"):
                ...
                for mode, sals in sorted(mode_salaries.items()):
                    mode_data.append({
                        "Modalidad": mode,
                        "Promedio": f"{sum(sals) // len(sals):,}E".replace(",", "."),
                        "Minimo": f"{min(sals):,}E".replace(",", "."),
                        "Maximo": f"{max(sals):,}E".replace(",", "."),
                        "Ofertas": len(sals),
                    })
            ...
            with st.expander("Salario por plataforma"):
                ...
                for src, sals in sorted(source_salaries.items(), key=lambda x: sum(x[1]) / len(x[1]), reverse=True):
                    src_data.append({
                        "Plataforma": src,
                        "Promedio": f"{sum(sals) // len(sals):,}E".replace(",", "."),
                        "Rango": f"{min(sals):,}E - {max(sals):,}E".replace(",", "."),
                        "Ofertas": len(sals),
                    })
            ...
            with st.expander("Salario por fuente (Directo vs Estimado)"):
                src_comp = []
                if direct_salaries:
                    src_comp.append({"Tipo": "Directo (de la oferta)", "Ofertas": len(direct_salaries), "Promedio": f"{sum(direct_salaries)//len(direct_salaries):,}E".replace(",", "."), "Minimo": f"{min(direct_salaries):,}E".replace(",", "."), "Maximo": f"{max(direct_salaries):,}E".replace(",", ".")})
                if estimated_salaries:
                    src_comp.append({"Tipo": "Estimado (IA)", "Ofertas": len(estimated_salaries), "Promedio": f"{sum(estimated_salaries)//len(estimated_salaries):,}E".replace(",", "."), "Minimo": f"{min(estimated_salaries):,}E".replace(",", "."), "Maximo": f"{max(estimated_salaries):,}E".replace(",", ".")})
                if src_comp:
        ...
        cv_skills_lower = set()
        profile_skills = latest.get("profile_skills", []) if isinstance(latest, dict) else []
        if profile_skills:
        ...
        mc3.metric("Salario mediano", f"{statistics.median(all_sal_data):,.0f}E".replace(",", ".") if all_sal_data else "N/A")
        ...
        if len(runs) > 1:
            ...
            if trend_data:
                ...
                with tc1:
                    st.markdown("**Salario promedio por ejecucion**")
                    sal_chart = trend_df.set_index("Fecha")[["Salario Promedio (E)"]]
                    st.line_chart(sal_chart)
                with tc2:
                    st.markdown("**% Remoto por ejecucion**")
                    remote_chart = trend_df.set_index("Fecha")[["% Remoto"]]
                    st.line_chart(remote_chart)
                st.markdown("**Ofertas encontradas por ejecucion**")
                offers_chart = trend_df.set_index("Fecha")[["Ofertas"]]
                st.bar_chart(offers_chart)
                with st.expander("Detalle por ejecucion"):
...
with tab_ejecuciones:
    ...
    k3.metric("Anadidas a Notion", added_to_notion)
    ...
    if len(runs) > 1:
        ...
        if history:
            ...
            chart_c1, chart_c2 = st.columns(2)
            with chart_c1:
                if "Fecha" in hist_df.columns:
                    chart_data = hist_df[["Fecha", "Encontradas", "Anadidas"]].copy() if "Anadidas" in hist_df.columns else hist_df[["Fecha", "Encontradas"]].copy()
                    chart_data = chart_data.set_index("Fecha")
                    st.line_chart(chart_data)
            with chart_c2:
                if "Fecha" in hist_df.columns:
                    chart_data2 = hist_df[["Fecha", "Scrapers OK", "Scrapers FAIL"]].copy() if "Scrapers FAIL" in hist_df.columns else hist_df[["Fecha"]].copy()
                    if len(chart_data2.columns) > 1:
