...
export async function POST(request: Request) {
  ...
  const workflow = body.workflow === "tenant-reanalyze.yml" || body.workflow === "tenant-scrape.yml" ? body.workflow : null;
  if (!workflow) return NextResponse.json({ error: "Workflow no permitido" }, { status: 400 });
  ...
  const inputs = { user_id: user.id, config_json: config };
